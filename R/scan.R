...
##'
##' The functions `scan_del` and `scan_find` are example functions
##' that delete and find all keys corresponding to a given pattern.
##'
##' @title Iterate over keys using `SCAN`
##'
##' @param con A [redis_api] object
##'
...
##'
##' @param ... additional arguments passed through to `callback`.
...
##'   is 10)
##'
##' @param type Type of `SCAN` to run.  Options are `"SCAN"` (the
##'   default), `"HSCAN"` (scan through keys of a hash), `"SSCAN"`
##'   (scan through elements of a set) and `"ZSCAN"` (scan though
##'   elements of a sorted set).  If `type` is not `"SCAN"`, then
##'   `key` must be provided.  `HSCAN` and `ZSCAN` currently do not
##'   work usefully.
##'
