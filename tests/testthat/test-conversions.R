...
test_that("difficult conversions", {
  ...
  expect_that(redis_flatten_command(x), equals(unlist(x, FALSE)))

  ## As generated by
  x <- list("foo", 1:2)
  expect_that(redis_flatten_command(x), equals(list("foo", 1:2)))
  expect_that(redis_check_command(x), equals(list("foo", c("1", "2"))))
})
