...
redux::hiredis()$DEL(c("mykey", "mylist", "mylist2", "myhash", "rlist"))
set.seed(1)
```
...
r$HGET("myhash", "myfield")
```

We can do this manually using `r$command()` and passing a list:

```{r}
r$command(list("HSET", "myhash", "myfield", "myvalue2"))
```
...
```{r}
r$command(list("HGET", "myhash", "myfield"))
```
